...
import { useItemComments, type ItemComment } from '@/hooks/use-item-comments';
...
// the withSpring configs some other sheets use. Used both for the open
// animation and for the pan gesture's "snap back to resting position" (a
// partial drag that didn't cross DISMISS_DISTANCE) — the same "settle at
// open position" motion either way, so both should feel identical.
...
// Bottom sheet for discussing one specific item — opened from the Comment
// icon in app/item/[id].tsx's ItemActionBar. Same slide/gesture/keyboard
// mechanics as components/collection/folder-comments-sheet.tsx (this is
// that component, retargeted at item_comments/useItemComments instead of
// folder_comments/useFolderComments — not a new interaction model). Slides
// up from the bottom on open; a downward swipe (or tapping the backdrop)
...
export function ItemCommentsSheet({ visible, onClose, itemId, itemTitle, currentUserId }: Props) {
  ...
  const { comments, loading, sending, addComment, deleteComment } = useItemComments(
    mounted ? itemId : undefined,
    currentUserId,
  );
              ...
              <View style={styles.fill}>
                {loading ? (
                  ...
                  <FlatList
                    ...
                    // "on-drag" rather than "interactive" — this whole sheet
                    // is already wrapped in its own GestureDetector/
                    // Gesture.Pan (above) for swipe-to-dismiss-the-sheet.
                    // "interactive" hands continuous touch tracking to the
                    // keyboard as the list is dragged, which would compete
                    // with that same drag for the pan-to-close gesture;
                    // "on-drag" still dismisses the keyboard on a scroll
                    // gesture without taking over the touch stream.
